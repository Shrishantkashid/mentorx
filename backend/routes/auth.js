...
router.post("/register", async (req, res) => {
  try {
    const { email: rawEmail, password, firstName, lastName, userType } = req.body;
    ...
    if (!isValidCampusEmail(email)) {
      return res.status(400).json({ success: false, message: "Please use a valid campus email address" });
    }
    ...
    const newUser = {
      ...
      mentor_approved: userType === 'mentor' ? false : undefined,
      created_at: new Date(),
      updated_at: new Date()
    };
    ...
    const token = jwt.sign(
      { userId: result.insertedId, email, userType: newUser.user_type },
    ...
    res.status(201).json({
      ...
      user: {
        ...
        mentorApproved: newUser.mentor_approved
      }
    });
  } catch (error) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user._id, email, userType: user.user_type || 'learner' },
      process.env.JWT_SECRET || "your-secret-key",
      { expiresIn: "7d" }
    );
